...
public abstract class AbstractGitFlowMojo extends AbstractMojo {
    ...
    protected void gitMerge(final String branchName, boolean rebase, boolean noff, boolean ffonly, String message,
            ...
            throws MojoFailureException, CommandLineException {
        gitMerge(branchName, null, rebase, noff, ffonly, message, messageProperties);
    }

    protected void gitMerge(final String branchName, String mergeStrategy, boolean rebase, boolean noff, boolean ffonly, String message,
            Map<String, String> messageProperties)
            throws MojoFailureException, CommandLineException {
        String sign = "";
        if (gpgSignCommit) {
        ...
        String mergeParams = "";
        if (StringUtils.isNotBlank(mergeStrategy)) {
            mergeParams = "--strategy=" + mergeStrategy;
        }
        if (rebase) {
            ...
            executeGitCommand("merge", mergeParams, "--ff-only", sign, branchName);
        } else if (noff) {
            getLog().info("Merging (--no-ff) '" + branchName + "' branch.");
            executeGitCommand("merge", mergeParams, "--no-ff", sign, branchName, msgParam, msg);
        } else {
            getLog().info("Merging '" + branchName + "' branch.");
            executeGitCommand("merge", mergeParams, sign, branchName, msgParam, msg);
        }
    }
