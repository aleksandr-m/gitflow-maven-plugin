...
public abstract class AbstractGitFlowMojo extends AbstractMojo {
	 ...
	 * Defaults to either "mvn" or "mvn.bat" depending on the operating system.
	 */
	@Parameter(property = "mvnExecutable")
	private String mvnExecutable;
	/**
	 * The path to the Git executable.
	 * Defaults to either "git" or "git.exe" depending on the operating system.
	 */
	...
	private void initExecutables() {
		if (StringUtils.isBlank(cmdMvn.getExecutable())) {
			if (StringUtils.isBlank(mvnExecutable)) {
				mvnExecutable = "mvn" + (Os.isFamily(Os.FAMILY_WINDOWS) ? ".bat" : "");
			}
	...
	protected boolean executeGitHasUncommitted() throws MojoFailureException, CommandLineException {
		...
		final String diffExitCode = executeGitCommandExitCode("diff", "--no-ext-diff", "--ignore-submodules", "--quiet", "--exit-code");

		if (SUCCESS_EXIT_CODE.equals(diffExitCode)) {
			// git diff-index --cached --quiet --ignore-submodules HEAD --
			final String diffIndexExitCode = executeGitCommandExitCode("diff-index", "--cached", "--quiet", "--ignore-submodules", "HEAD", "--");
	 ...
	 * @throws CommandLineException
	 */
	private void gitSetConfig(final String name, final String value) throws MojoFailureException, CommandLineException {
	 ...
	 * @throws CommandLineException
	 */
	protected String gitFindBranches(final String branchName) throws MojoFailureException, CommandLineException {
		String pattern = "refs/heads/" + branchName + "*";
		String branches = executeGitCommandReturn("for-each-ref", "--format=\"%(refname:short)\"", pattern);

		// on *nix systems return values from git for-each-ref are wrapped in quotes
	...
	protected void gitCreateAndCheckout(final String newBranchName, final String fromBranchName) throws MojoFailureException, CommandLineException {
		getLog().info(msg.getMessage(LogMessages.checking_out_new_branch, newBranchName, fromBranchName));
	 ...
	 * @throws CommandLineException
	 */
	protected void gitTag(final String tagName, final String message) throws MojoFailureException, CommandLineException {
	 ...
	 * @throws MojoFailureException
	 */
	private String executeGitCommandExitCode(final String... args) throws CommandLineException, MojoFailureException {
	 ...
	 * @param returnOut Whether to return output. When <code>true</code> the
	 *        output will not be printed into the console and will be returned
	 *        from this method.
	 * @param returnExitCode If <code>true</code> the exit code of the command
	 *        will be returned, if <code>false</code> the output of the command
	 *        will be returned. Will not have effect if the
	 ...
	 * @throws CommandLineException
	 * @throws MojoFailureException If <code>returnExitCode</code> is
	 *         <code>false</code> and command exit code is NOT equals to 0.
	 */
	private String executeCommand(final Commandline cmd, final boolean returnOut, final boolean returnExitCode, final String... args)
